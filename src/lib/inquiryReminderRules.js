 ...
 */
export async function runInquiryReminderRulesForInquiry(inquiry) {
  if (!inquiry?.id) {
    return { status: 'skipped', action: null, reason: 'no_inquiry_id' };
  }
...
export async function runInquiryReminderRulesForAll() {
  ...
  try {
    inquiries = await base44.entities.Inquiry.list();
  } catch (error) {
    console.error('[InquiryReminderRules] failed to load inquiries', error);
    summary.errors += 1;
    return summary;
  }
  ...
  for (const inquiry of inquiries) {
    ...
    try {
      const result = await runInquiryReminderRulesForInquiry(inquiry);
      ...
      if (result.action === 'created') {
        ...
        summary.resolved += 1;
      } else if (result.status === 'skipped' || result.action === 'not_found') {
        // already counted skipped when applicable
      }
    } catch (error) {
      summary.errors += 1;
      console.error(
